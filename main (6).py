'''
Written by Jyo Pari and Joseph L. Hora

Parts of the code for using MobileNets were taken from:
...
print(path, findlast(path,'/'))

#creating the region file and the catalog file
regionfile = open('results/'+path[(findlast(path,'/')+1):len(path)-15]+'.reg','w+')
catalog = open('results/'+path[(findlast(path,'/')+1):len(path)-15]+'_catalog.txt','w+')
...
def convertLat(lat): 
	lat = str(format(round(float(lat),2),'.2f'))
	pos = lat.find('.')
	a = lat[pos+1]
	b = lat[pos+2]
	x = '0'
...
#converts the Lon coordinate into format of "(+,-)00.000"
def convertLon(lon_og): 
	lon = str(format(round(float(lon_og),3),'.3f'))
	pos = lon.find('.')
	a = lon[pos+1]
	b = lon[pos+2]
	c = lon[pos+3]
...
img = img.point(lambda i: i * float(scalefactor))
...
# replace minimum flux values that are less than zero
	if(minimum < 0.0):
		minimum = 0.01
	median = np.median(values)
	i.append((median-minimum)/median)
	i.append(area)
...
for c in newbox:
	...
	if(c[2]*c[3] < 30000):  
		...
		if(results[1]>float(small_thresh)):
			print('G'+convertLat(lat)+convertLon(lon)+' '+str(format(lat, '.4f'))+' '+str(format(lon, '.4f'))+' '+str(format(width, '.1f'))+' '+str(format(height, '.1f'))+' '+str(format(c[6], '.2f'))+' '+str(format(results[1],'.2f'))+' '+str(format(c[7], '.0f')))
			num_small_irdc += 1
			regionfile.write('Galactic;'+' box '+str(lat)+' '+str(lon)+' '+str(width_degrees)+' '+str(height_degrees)+'\n')
			catalog.write('G'+convertLat(lat)+convertLon(lon)+' '+str(format(lat, '.4f'))+' '+str(format(lon, '.4f'))+' '+str(format(width, '.1f'))+' '+str(format(height, '.1f'))+' '+str(format(c[6], '.2f'))+' '+str(format(results[1],'.2f'))+' '+str(format(c[7]*1.44, '.0f'))+'\n')
